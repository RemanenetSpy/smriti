...
// ── Quick-action cards shown below the guide ───────────────────────────────────
const ACTIONS = [
  {
    id: "ingest",
    Icon: Activity,
    label: "Ingest an Event",
    sub: "Send text → Smriti stores it as structured memory",
  },
  {
    id: "query",
    Icon: Search,
    label: "Query Memory",
    sub: "Ask a question → get ranked results across all events",
  },
  {
    id: "agent",
    Icon: Cpu,
    label: "Run Agent",
    sub: "Let the AI reason across your full memory context",
  },
];

// ── Collapsed quick-start bar ──────────────────────────────────────────────────
function QuickStart() {
              ...
              <div key={step.id}>
                ...
                >
                  <div
                    ...
                    onKeyDown={(e) => {
                      if (e.key === " " || e.key === "Enter") toggleDone(step.id, e as any);
                    }}
                  ...
                  <span className={`text-sm flex-1 ${isDone ? "text-[#bbb] line-through" : "text-black"}`}>
                    {step.title}
                  </span>
                ...
                {isOpen && (
                  <div className="px-5 pb-4 ml-[52px] space-y-2">
...
export function Overview({
  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-8 space-y-6">
      ...
      {/* No API key — guide user to enter one */}
      {!apiKey && (
        <div className="flex items-center justify-between px-4 py-3 bg-[#fafafa] border border-[#e0e0e0] rounded-lg">
          <div className="flex items-center gap-2.5">
            <div className="w-1.5 h-1.5 rounded-full bg-[#f59e0b]" />
            <span className="text-sm text-black font-medium">No API key set</span>
            <span className="text-sm text-[#999]">— enter yours to connect Smriti</span>
          </div>
          <button
            onClick={() => onNavigate?.("keys")}
            className="text-xs font-medium text-black underline underline-offset-2 hover:opacity-60 transition-opacity"
          >
            Go to API Keys →
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          ...
          <div className="border border-[#eaeaea] rounded-lg p-4 bg-white hover:border-black transition-colors">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-sm font-medium text-black">{health ? "Operational" : "…"}</span>
            </div>
          ...
          {/* Engine */}
          <div className="border border-[#eaeaea] rounded-lg p-4 bg-white hover:border-black transition-colors">
            <div className="text-sm font-medium text-black">GPT OSS 120B</div>
      ...
      {/* Quick Actions — fills space, drives navigation */}
      <div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {ACTIONS.map(({ id, Icon, label, sub }) => (
            <button
              ...
              className="border border-[#eaeaea] rounded-lg p-4 bg-white hover:border-black hover:shadow-sm transition-all text-left group"
            >
              <div className="flex items-start justify-between mb-3">
                <div className="w-8 h-8 rounded-md border border-[#eaeaea] flex items-center justify-center group-hover:border-black transition-colors">
                  <Icon className="w-3.5 h-3.5 text-black" strokeWidth={1.5} />
                </div>
                <ArrowRight className="w-3.5 h-3.5 text-[#ccc] group-hover:text-black transition-colors mt-0.5" />
              </div>
